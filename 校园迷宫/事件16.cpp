...
void shijian16(int a[32][18], int* x1, int* y1, int* x2, int* y2, int b[17]) {
	...
	_tcscpy_s(p[0], L"陶行染：这是什么地方？仓库吗？");
	_tcscpy_s(p[1], L"陶行染：这么大个地方位置倒是真的偏。");
	_tcscpy_s(p[2], L"陶行染：打不开就算了吧，还有人追着我呢。");
	_tcscpy_s(q[0], L"陶行染：这地方，不会能用教室卡打开吧。");
	_tcscpy_s(q[1], L"陶行染：还真能。");
	_tcscpy_s(q[2], L"仓库门打开了。");
